...
const Users = () => {
  ...
  const { mutate: mutateUpdateUserLevel } =
    useMutation({
      mutationFn: ({ subscribe_level, userId }) => {
        return updateProfile({
          token: userState.userInfo.token,
          userData: { subscribe_level: subscribe_level },
          userId,
        });
      },
      onSuccess: (data) => {
  ...
  const handleLevelCheck = (event, userId) => {
    if (
      window.confirm("Do you want to change the level subscribe user?")
    ) {
      mutateUpdateUserLevel({ subscribe_level: event, userId });
    }
  };
      ...
      {usersData?.data.map((user) => (
        <tr key={user._id}>
          ...
          <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
            <p className="text-gray-900 whitespace-no-wrap">
              <CreatableSelect
                defaultValue={optionsSubscribeLevel.find(item => user.subscribe_level === item.value)}
                options={optionsSubscribeLevel}
                onChange={(newValue) =>
                  handleLevelCheck(newValue.value, user._id)
                }
              />
            </p>
          </td>
